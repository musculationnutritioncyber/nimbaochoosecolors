...
export async function saveImage(id, buffer, contentType) {
  ...
  const blob = await put(`img/${id}.${ext}`, buffer, { access: 'public', addRandomSuffix: true, contentType });
  return blob.url;
}
...
function aggPrefix() {
  const key = crypto.createHash('sha256').update(`agg:${process.env.ADMIN_PIN || ''}`).digest('hex').slice(0, 24);
  return `agg/${key}/`;
}
