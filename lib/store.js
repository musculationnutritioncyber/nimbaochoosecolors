...
function aggPrefix() {
  const key = crypto.createHash('sha256').update(`agg:${ADMIN_PIN}:${deck().countFrom || 0}`).digest('hex').slice(0, 24);
  return `agg/${key}/`;
}
...
function fold(state, pathname, batch) {
  ...
  if ((batch.t || 0) < (deck().countFrom || 0)) return; // cast before the last reset (deck.json countFrom)
